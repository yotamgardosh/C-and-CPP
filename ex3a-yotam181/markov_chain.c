...
first_node, int max_length)
{
  ...
  tweet[0] = '\0'; // add null terminator so strcat works
  ...
  strcat (tweet, word_node->data);
  while ((!(is_end_of_sentence(word_node->data))) && count < max_length)
  {
    strcat (tweet, " ");
...
/**
   returns true if the given word string ends with a period ('.'),
   false otherwise.
 - for full documentation see header file
 */
bool is_end_of_sentence(char* word)
{
  unsigned long last_char = strlen (word) - 1;
  if(word[last_char] == '.')
  {
    return true;
  }
...
MarkovNode* get_first_random_node(MarkovChain *markov_chain)
{
  ...
  while (true)
  {
    if(count == random_num)
    {
      if(is_end_of_sentence(trav->data->data))
      {
...
/**
 ...
 - for full documentation see header file
 */
char* new_data(char** word, int word_length, int max_length)
{
  char* data = malloc (max_length);
  if(!data)
  {
    return NULL;
  }
...
Node* add_to_database(MarkovChain *markov_chain, char *data_ptr)
{
  ...
  while(ptr_trav != NULL)
  {
    if(strcmp(ptr_trav->data->data,data_ptr) == 0)
    {
      free (data_ptr);
      return ptr_trav;
    }
  ...
  MarkovNode* m_node = new_MarkovNode(data_ptr);
  if(!m_node)
  {
    return NULL;
  }
...
bool add_node_to_frequencies_list(MarkovNode *first_node, MarkovNode *second_node)
{
  ...
  {
    first_node->frequencies_list = new_MarkovNodeFrequency (second_node);
    if(first_node->frequencies_list == NULL)
    {
      return false;
    }
  ...
  MarkovNodeFrequency* node_to_add = new_MarkovNodeFrequency (second_node);
  if(!node_to_add)
  {
    return false;
  }
...
{
  MarkovChain* chain_to_free = *ptr_chain; //maybe wrong type
  ...
  free(chain_to_free->database); // free linked list
...
//{
//  Node* trav = markov_chain->database->first;
//  while(trav)
//  {
//    printf ("cur node: %s\n", trav->data->data);
//    printf ("freq_list:\n");
//    for(int i =0; i < trav->data->f_list_size; i++)
//    {
//      printf ("%s -- ",trav->data->frequencies_list[i].markov_node->data);
//      printf ("%d\n",trav->data->frequencies_list[i].frequency);
//    }
//    printf ("-----------------------\n");
//    trav = trav->next;
//  }
