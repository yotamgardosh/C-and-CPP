...
int main (int argc, char *argv[])
{
  ...
  fclose (text_corpus);
  if(filled_bool == EXIT_FAILURE){ // README - changed
    printf(ALOC);
    free_database(&mc);
    return EXIT_FAILURE;
  }
...
int fill_database(FILE *fp, int words_to_read, MarkovChain *markov_chain)
{
  ...
  void* data = new_data (&parsed_word, strlen(parsed_word) + 1, MAX_LENGTH);
  if(!data)
  {
    return EXIT_FAILURE;
  }
  Node* added1 = add_to_database (markov_chain, data);
  free(data); //ADDED TO FIX VALGRIND ISSUE
  data = NULL;
  if(!added1)
  {
    free(parsed_word); //ADDED TO FIX VALGRIND ISSUE take 2
    return EXIT_FAILURE;
  }
  word1 = added1->data;
  ...
  while(words_to_read && flag)
  {
    while (words_to_read)
    {
      ...
      word2 = added2->data;
      add_node_to_frequencies_list (word1, word2, markov_chain);
      word1 = word2;
      words_to_read = update_words_to_read (words_to_read);
    }
...
/**
   returns true if the given data ends with a period ('.'),
   false otherwise.
...
bool is_end_of_sentence(const void* data)
{
  ...
  unsigned long last_char = strlen ((char*)word) - 1;
  if(word[last_char] == '.')
  {
    return true;
  }
...
void print_func(void* data)
{
  char* word_to_print = data;
  if(is_end_of_sentence (word_to_print))
  {
...
int comp_func(const void* data1, const void* data2)
{
  const char* word1 = data1;
  const char* word2 = data2;
  return strcmp (word1, word2);
}

void* copy_func(const void* data)
{
  const char* word_to_copy = data;
  char* copy_to = malloc ((strlen (word_to_copy) + 1));
  if(!copy_to)
  {
    return NULL;
  }
  strcpy (copy_to, word_to_copy);
